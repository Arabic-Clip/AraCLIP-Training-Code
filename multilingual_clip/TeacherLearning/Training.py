...
# If you want to use fp16, uncomment the following lines and set precision_16 to True
# precision = "mixed_float16"
# mixed_precision.set_global_policy(precision)

def loadTargetEmbeddings(dataset_name_train,dataset_name_validation):

    trainSamples = datasets.load_dataset(dataset_name_train, split='train')
    valSamples = datasets.load_dataset(dataset_name_validation, split='validation') 

    
    logger.info(f"len(trainSamples): {len(trainSamples)}") # len(trainSamples) 2000000
...
def singleGPUTraining():
    ...
    stepsPerEpoch, lr = 15625, 0.000001  # 566405/128 = 4425.0390625 # 586, 0.00001 # maximum number of stepPerEpoch I can feed: 585.9375
    gradAccumSteps, batchSize = 1, 128 # 1, 2 # 1, 128 # 256
    epochs = 200
    numTrainSteps, numWarmupSteps = 3125000, 1000 # 1
    
    modelBase = "aubmindlab/bert-large-arabertv2" 
    tokenizerBase = "aubmindlab/bert-large-arabertv2"

    imageBase = "ViT-B-16-SigLIP-512"
    modelName = "arabertv2" +  "-" + imageBase + "-" 

    log_name =  "arabertv2" +  "-" + imageBase + "-" 
    
    startWeights = None # "/ARBERTv2_vit_B_16_plus/ARBERTv2-Vit-B-16-plus-240- 103_2023 11 11 - 22 18 20_epoch_103_internal_.keras"

    dataset_name_validation = "Arabic-Clip/Arabic_3M_5M_ViT-B-16-plus-240"
    dataset_name_train  = "Arabic-Clip/Arabic_3M_5M_ViT-B-16-plus-240" 
    ...
    if (startWeights is not None):
        ...
        pickle_file_path = "/home/lenovo/Desktop/arabic_clip/ARBERTv2_vit_B_16_plus/ARBERTv2_vit_B_16_plusheads_of_the_model_ARBERTv2-Vit-B-16-plus-240-103_.pickle" 
